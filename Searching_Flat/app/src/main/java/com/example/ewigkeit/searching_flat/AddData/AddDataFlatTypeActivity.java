...
public class AddDataFlatTypeActivity extends AppCompatActivity{
    @BindView(R.id.description_flattype)
    EditText description;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_add_flattype);
        ButterKnife.bind(this);
    }

    @OnClick(R.id.save_flattype)
    public void onSaveClick() {
        DatabaseHelper databaseHelper = App.getDatabaseInstance();
        FlatType model = new FlatType(description.getText().toString());
        if(databaseHelper.getFlatTypeDao().getFlatTypeEquals(description.getText().toString())==null) {

            databaseHelper.getFlatTypeDao().insert(model);
            ...
            // set dialog message
            alertDialogBuilder
                    .setMessage("Такой тип квартиры уже существует")
                    .setCancelable(false)
